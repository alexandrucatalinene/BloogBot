...
namespace BloogBot.AI.SharedStates
{
    public class LootState : IBotState
    {
        ...
        private bool CanBeSkinned()
        {
            return target.CanBeSkinned && Inventory.GetAllItems().Any(x => x.Info.Name == "Skinning Knife");
        }
        ...
        public void Update()
        {
            ...
            if (target.CanBeLooted && currentState == LootStates.Initial && player.Position.DistanceTo(target.Position) < 5)
            {
                ...
                if (Wait.For("StartLootDelay", 1000))
                {
                    target.Interact();
            ...
            if(CanBeSkinned() && currentState == LootStates.Initial)
            {
                player.StopAllMovement();

                if (Wait.For("StartLootDelay", 3500))
                {
                    target.Interact();
                    currentState = LootStates.RightClicked;
                    return;
            ...
            if ((currentState == LootStates.Initial && !target.CanBeLooted) || (lootFrame != null && lootIndex == lootFrame.LootItems.Count) || stuckCount > 5 || Environment.TickCount - startTime > 20000)
            {
                player.StopAllMovement();
                botStates.Pop();
            ...
            if (currentState == LootStates.LootFrameReady && Wait.For("LootDelay", 300))
            {
                ...
                if (lootIndex == lootFrame.LootItems.Count)
                    currentState = LootStates.Initial;
            }
        }
    }
